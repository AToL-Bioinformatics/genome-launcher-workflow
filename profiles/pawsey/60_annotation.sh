...
(
	XDG_CACHE_HOME="$(mktemp -d)" \
		...
		--cluster-generic-submit-cmd "mkdir -p logs/slurm/{rule} && sbatch --time={resources.runtime} --partition=gpu --account=pawsey1132-gpu --gres=gpu:{resources.gpu} --job-name={rule}-smk --output=logs/slurm/{rule}/{rule}-%j.out --parsable" \
		tiberius
)
